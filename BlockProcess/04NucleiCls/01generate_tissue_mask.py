...
if __name__ == "__main__":
    ...
    for ind, cur_core in enumerate(core_lst):
        ...
        # initiaite the map
        lowlevel_w, lowlevel_h = slide_head.level_dimensions[args.slide_level]
        cell_map = np.zeros((lowlevel_h, lowlevel_w), dtype=np.uint32)
        divide_ratio = np.power(2, args.slide_level)
        ...
        # save cell_mask
        tissue_mask_path = os.path.join(slide_tissue_dir, cur_core + "-L" + str(args.slide_level) + "-Tissue.png")
