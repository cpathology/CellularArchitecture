...
def set_args():
    parser = argparse.ArgumentParser(description = "Seg and merge block-wise segmentation")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(block_img_dir):
        sys.exit("{} directory not exist.".format(block_img_dir))
    slide_lst = sorted([ele for ele in os.listdir(block_img_dir) if os.path.isdir(os.path.join(block_img_dir, ele))])    
    if len(slide_lst) == 0:
        sys.exit("No available SlideBlocks inside folder {}".format(block_img_dir))
    else:
   ...
   # model setting
    checkpoint_dir = os.path.join(args.data_root, args.checkpoint_dir, "NucleiSeg")
